...
import VkontakterCMultipartParser
...
public final class MultipartParser {
    ...
    private var callbacks: vkmultipartparser_callbacks
    private var parser: vkmultipartparser
    ...
    public init(boundary: String) {
        ...
        var parser = vkmultipartparser()
        t_vkmultipartparser_init(&parser, boundary)
        var callbacks = vkmultipartparser_callbacks()
        t_vkmultipartparser_callbacks_init(&callbacks)
    ...
    struct Context {
        static func from(_ pointer: UnsafeMutablePointer<vkmultipartparser>?) -> Context? {
    ...
    public func execute(_ buffer: ByteBuffer) throws {
        let result = buffer.withUnsafeReadableBytes { (unsafeBuffer: UnsafeRawBufferPointer) -> Int in
            ...
            return withUnsafeMutablePointer(to: &context) { (contextPointer: UnsafeMutablePointer<Context>) -> Int in
                ...
                return t_vkmultipartparser_execute(&self.parser, &self.callbacks, unsafeBuffer.baseAddress?.assumingMemoryBound(to: Int8.self), unsafeBuffer.count)
            }
        }
